...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $formPage === 'signUp') {
  ...
  if (empty($address)) {
    ...
    $errorsMessage['address'] = "Alamat harus sesuai.";
  }
  ...
  // Cek apakah ada error sebelum menyimpan ke database
  if (empty($errorsMessage)) {
    // Simpan data ke database
    $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

    $stmt = $pdo->prepare("INSERT INTO users (fullName, email, date_of_birth, balance, password, address)
        VALUES (?, ?, ?, ?, ?, ?)");
    $executeSucces = $stmt->execute([$fullName, $email, $date_of_birth, $balance, $hashedPassword, $address]);
          ...
          <form action="loginPage.php?form=signUp" method="post">
            ...
            <div class="row">
              <div class="input-group">
                ...
                <input type="date" name="dob" id="dob" value="<?php echo htmlspecialchars($date_of_birth ?? ''); ?>"
                  required>
            ...
            <div class="row">
              <div class="input-group">
                ...
                <input type="text" name="address" id="address" placeholder="masukkan kota dan nama Jalan anda"
